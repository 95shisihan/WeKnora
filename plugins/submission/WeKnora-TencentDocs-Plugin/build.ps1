...
try {
    ...
    $stage = Join-Path $PSScriptRoot 'dist/tencent-docs-public-0.2.0'
    New-Item -ItemType Directory -Force (Join-Path $stage 'bin') | Out-Null
    & $Go build -mod=mod -trimpath -o (Join-Path $stage 'bin/weknora-tencent-docs-public.exe') .
    if ($LASTEXITCODE -ne 0) { throw 'Plugin build failed' }
    Copy-Item -LiteralPath 'plugin.yaml' -Destination (Join-Path $stage 'plugin.yaml')
    $archive = Join-Path $PSScriptRoot 'dist/tencent-docs-public-windows-x64-0.2.0.zip'
