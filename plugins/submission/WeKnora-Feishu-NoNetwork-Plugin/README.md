> 独立构建与目录变更见 [MIGRATION.md](MIGRATION.md)。
...
生成的 ZIP 位于 `dist/`，包含清单和独立 EXE，无需安装 Python 或 Docker。

要运行真实飞书/系统权限对照验收：

```powershell
# 先在本仓库运行 ./build.ps1，再在宿主的 Go/CGO 环境执行：
$env:WEKNORA_WINDOWS_SANDBOX_TEST = '1'
$env:WEKNORA_FEISHU_DENIED_EXE = '<本仓库生成EXE的绝对路径>'
$env:WEKNORA_FEISHU_TEST_URL = '<可访问的飞书公开链接>'
go test ./internal/plugin -run '^TestNativeFeishuConnectionDenied$' -count=1
```
