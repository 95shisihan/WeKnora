...
try {
    ...
    $stage = Join-Path $PSScriptRoot 'dist/local-directory-0.1.0'
    New-Item -ItemType Directory -Force (Join-Path $stage 'bin') | Out-Null
    & $Go build -mod=mod -trimpath -o (Join-Path $stage 'bin/weknora-plugin-local-directory.exe') .
    if ($LASTEXITCODE -ne 0) { throw 'Plugin build failed' }
    Copy-Item -LiteralPath 'plugin.windows.yaml' -Destination (Join-Path $stage 'plugin.yaml')
    $archive = Join-Path $PSScriptRoot 'dist/local-directory-windows-x64-0.1.0.zip'
