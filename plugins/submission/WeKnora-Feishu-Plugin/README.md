> 独立仓库 `WeKnora-Feishu-Plugin`，构建只读取本仓库 SDK。Windows 公开链接版本执行 `python build_windows.py --public`，知识库模式去掉 `--public`。
...
```powershell
$env:WEKNORA_PLUGIN_DIRS = (Resolve-Path '<插件制品目录>').Path
