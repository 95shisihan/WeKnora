...
PLUGIN_ID = os.getenv("WEKNORA_PLUGIN_ID", "dev.example.local-directory")
PLUGIN_VERSION = os.getenv("WEKNORA_PLUGIN_VERSION", "0.1.0")
CONNECTOR_TYPE = "example_local_directory"
...
def config_root(raw: bytes) -> Path:
    try:
        ...
        root = Path(config.get("settings", {}).get("root", "/data")).resolve()
    except (ValueError, TypeError) as exc:
        raise ValueError(f"invalid config: {exc}") from exc
    if root != ALLOWED_ROOT and ALLOWED_ROOT not in root.parents:
        raise ValueError("root must be /data or one of its descendants")
    if not root.is_dir():
...
def main() -> None:
    address = os.getenv("WEKNORA_PLUGIN_ADDRESS", "unix:///tmp/weknora-plugin.sock")
