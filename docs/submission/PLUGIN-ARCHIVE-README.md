...
主仓通过 GitHub 链接及最终 Tag 提交；独立插件不发布远程仓库，源码、构建脚本、Manifest、README、已有依赖锁定文件和选定的 Windows 安装包均归档在主仓库 `plugins/submission/`。
...
解压后按各插件 README 构建。主验收是本地目录 Python 插件，其 Windows 安装包位于 `plugins/WeKnora-LocalDirectory-Plugin/dist/independent-directory-windows-x64-0.1.0.zip`。其他插件安装包位于对应 `dist/`，专有模型包位于该插件根目录。应在 GitHub 目录中下载对应的单个插件安装包，再上传到插件管理；不能上传整个 `plugins/submission` 目录。
