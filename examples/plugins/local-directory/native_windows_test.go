...
func TestNativeDirectoryPluginHelper(t *testing.T) {
	if os.Getenv("WEKNORA_DIRECTORY_HELPER") != "1" {
		return
	}
	main()
	os.Exit(0)
...
func TestNativeDirectoryStdioIncremental(t *testing.T) {
	if os.Getenv("WEKNORA_WINDOWS_SANDBOX_TEST") != "1" {
		t.Skip("native Windows acceptance is opt-in")
	}
	dir := t.TempDir()
	...
	p, err := windowsandbox.Start(windowsandbox.Config{
		Command: []string{exe, "-test.run=^TestNativeDirectoryPluginHelper$"}, Directory: dir, ConfigRead: []string{"settings.root"}, Stderr: os.Stderr,
		Environment: append(os.Environ(), "WEKNORA_DIRECTORY_HELPER=1", "WEKNORA_PLUGIN_ADDRESS=stdio://", "WEKNORA_PLUGIN_ID=io.weknora.local-directory", "WEKNORA_PLUGIN_VERSION=0.1.0"),
	})
	require.NoError(t, err)
	defer p.Close()
