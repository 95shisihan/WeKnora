...
try {
    ...
    $stage = Join-Path $repo 'artifacts/controlled-http-windows-x64-0.1.0'
    New-Item -ItemType Directory -Force (Join-Path $stage 'bin') | Out-Null
    & $go build -o (Join-Path $stage 'bin/weknora-controlled-http.exe') ./examples/plugins/controlled-http
    if ($LASTEXITCODE -ne 0) { throw 'Plugin build failed' }
    Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'plugin.yaml') -Destination $stage
    $zip = Join-Path $repo 'artifacts/plugins/controlled-http-windows-x64-0.1.0.zip'
